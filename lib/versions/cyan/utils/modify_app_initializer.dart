...
void _injectFirebaseInitialization(String projectName, bool hasFlavors) {
  ...
  if (!appInitializerFile.existsSync()) {
    print('  ⚠️  lib/_app_initializer.dart not found, skipping code injection...');
    return;
  }
  ...
  if (content.contains('Firebase.initializeApp')) {
    print('  ℹ️  Firebase initialization already exists in _app_initializer.dart');
    return;
  }
  ...
  if (match == null) {
    print('  ⚠️  Could not find initializeApp() function in _app_initializer.dart');
    return;
  }
  ...
  print('  ✓ Injected Firebase initialization code into _app_initializer.dart');
